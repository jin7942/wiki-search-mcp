...
서브폴더 계층화·구조 건강 진단·파일명 정규화를 제안하는 read-only 도구들을
추가했다. 테스트 739 → 780.

### Added

- **서브폴더 계층화 제안 (`wiki_suggest_subfolders`)**: 파일이 누적된 평면
  프로젝트 폴더에 대해 frontmatter 태그 + 본문 키워드를 공통 신호로 묶어
  서브폴더 구조를 제안한다. ``min_cluster_size`` 이상 묶이는 신호만 후보가
  되고 greedy 선점(큰 그룹 우선, 중복 할당 없음)으로 한 파일은 한 그룹에만
  든다. 제안만 반환하며 폴더 생성/이동은 하지 않는다(read-only 정책 유지).
- **구조 건강 진단 (`wiki_health_check`)**: 평면 누적 폴더(직계 .md 가 임계
  이상인데 서브폴더 없음)와 빈 폴더(하위에 .md 가 전무)를 한 번에 진단한다.
  계층화/정리 대상을 사용자에게 제시(read-only).
- **파일명 날짜 정규화 제안 (`wiki_suggest_filename_normalization`)**: 파일명
  선두의 제각각인 날짜 표기(``2026.05.12``/``26.05.11``/``260513`` 등)를 표준
  ``YYYY-MM-DD`` 로 통일하는 rename 후보를 제시한다. 월/일 범위 검증으로 날짜가
  아닌 숫자 오탐을 막는다. 제안만 반환(실제 rename/링크 갱신은 LLM 이 수행).
- **폴더 경로 검증 (`validate_dir_path`)**: ``.md`` 강제 정규화 없이 traversal
  방어만 수행. 폴더를 인자로 받는 유스케이스용.

### Changed

- **분류 추천 닭-달걀 해소**: ``DocumentService.get_similar`` 가 인덱스 벡터
  부재 시(신규/이동 직후 미인덱싱 파일) 디스크 본문을 즉석 임베딩해 유사 문서를
  찾는다. ``embedder`` 옵셔널 주입(미주입 시 기존 동작 보존).
- **카테고리 후보 신호화**: ``_compute_category_candidates`` 가 신호(폴더/유사
  문서 투표) 후보와 정적 보조를 구분하고, 신호가 0이면 추천 근거에 "신호
  없음(활성 목록)" 을 명시한다(과거엔 신호 없어도 정적 5개를 그대로 반환해
  모든 파일이 동일 후보로 수렴).
